document.querySelectorAll('input[name="currentCoordType"]').forEach((radio) => {
	radio.addEventListener('change', function () {
		const selectedSystem = this.id.replace('current-', '');
		alert(`当前选择的坐标系: ${selectedSystem.toUpperCase()}`);
	});
});
